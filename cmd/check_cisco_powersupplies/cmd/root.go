...
func hardCodedWalk(params *g.GoSNMP) {
	...
	result, err := common.BulkWalkToMap(params, entPhysicalDescrOID)
	if err != nil {
		log.Fatalf("Error: %v\n", err)
		return
	}
	...
	result, err = common.BulkWalkToMap(params, entPhysicalClassOID)
	if err != nil {
		log.Fatalf("Error: %v\n", err)
		return
	}
	...
	if psuBuiltIn {
		...
		result, err = common.BulkWalkToMap(params, ciscoEnvMonSupplyStateOID)
		if err != nil {
			log.Fatalf("Error: %v\n", err)
			return
		}
		...
		result, err = common.BulkWalkToMap(params, cefcFruPowerOperStatusOID)
		if err != nil {
			log.Fatalf("Error: %v\n", err)
			return
		}
